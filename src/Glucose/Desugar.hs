...
desugar :: Error m => AST.Module FromSource -> m (IR.Module Unchecked)
desugar (AST.Module defs) = IR.Module . concat <$> traverse definition defs

definition :: Error m => FromSource (AST.Definition FromSource) -> m [Desugared IR.Definition]
...
expression :: Error m => AST.Expression FromSource -> m (IR.Expression Unchecked)
expression (AST.Value a) = value a
expression (AST.Apply f a) = IR.Apply <$> traverse expression f <*> traverse value a

value :: Error m => AST.Value FromSource -> m (IR.Expression Unchecked)
