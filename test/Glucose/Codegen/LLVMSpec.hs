...
spec = describe "LLVM codegen" $ do
  it "compiles an empty module" $
    codegen (fromDefinitions []) `shouldBe` LLVM.Module []
  ...
  it "compiles global aliases correctly" $
    codegen (fromDefinitions [aliasAnywhere "a" "b" Integer]) `shouldBe`
      LLVM.Module [LLVM.Alias (mkName "a") (mkName "b") LLVM.TI32]
  it "compiles enum constructors correctly" $
    codegen (fromDefinitions [constructorAnywhere "test" "a" 0, constructorAnywhere "test" "B" 1]) `shouldBe`
  ...
  it "mangles global identifiers" $
    codegen (fromDefinitions [constantAnywhere "-._$\x05d0\&azAZ09\x5d5_" $ IR.IntegerLiteral 0]) `shouldBe`
